...
type Config struct {
	...
	// Email-form verification: a hosted page the insurer fills in, emailed to
	// payers with no real-time EDI and no phone line (or as an alternate channel).
	EmailFormBaseURL     string        // public origin this backend is reachable at, e.g. https://verafy.example.com
	EmailResponseTimeout time.Duration // EMAIL_PENDING longer than this -> manual review
...
func Load() (*Config, error) {
	c := &Config{
		...
		EmailFormBaseURL:     env("EMAIL_FORM_BASE_URL", "http://localhost:8080"),
		EmailResponseTimeout: envDur("EMAIL_RESPONSE_TIMEOUT", 72*time.Hour),
